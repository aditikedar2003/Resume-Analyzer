...
def detect_contact_info(resume_text: str):
    ...
    if re.search(r"\b(city|state|province|street|road|ave|avenue|lane|pune|mumbai|delhi|bangalore|bangalore|hyderabad)\b", resume_text.lower()) or re.search(r"\b\d{5,6}\b", resume_text):
        info["address"] = True
...
def categorize_and_compare(resume_text: str, jd_text: str):
    ...
    def present_set(req_set, source_counts):
        ...
        for kw in sorted(req_set):
            ...
            # check if any token sequence exists in resume / jd counts
            joined = " ".join(parts)
...
def compute_weighted_score(cat_compare):
    ...
    w_soft = 0.2
    w_verb = 0.1
    w_sections = 0.15  # later adjusted with section presence
    # counts
...
def render_chip_html(label, count, color="#f0f0f0", link_id=None):
    style = f"background:{color}; padding:10px 14px; border-radius:10px; font-weight:700; display:inline-block; cursor:pointer; margin-right:12px;"
    if link_id:
...
div.stButton > button { background-color: #6c63ff; color: white !important; padding: 8px 16px; border-radius:8px; border:none; font-weight:600; min-width:100px; }
div.stButton > button:hover { filter:brightness(1.05); }
.content-wrap { max-width:1100px; margin:28px auto; padding: 0 18px; }
.center-card { text-align:center; padding:26px; background:#fbfcff; border-radius:10px; border:1px solid #eef2ff; }
.metric-row { display:flex; gap:12px; flex-wrap:wrap; justify-content:center; margin-bottom:16px; }
.section-box { padding:14px; border-radius:8px; border:1px solid #eee; background:#fff; margin-bottom:12px; }
.kv { font-weight:600; margin-bottom:6px; }
.small { color:#666; font-size:14px; }
.badge-good { color:#155724; background:#e6f4ea; padding:6px 10px; border-radius:8px; display:inline-block; }
.badge-warn { color:#856404; background:#fff3cd; padding:6px 10px; border-radius:8px; display:inline-block; }
.badge-bad { color:#721c24; background:#f8d7da; padding:6px 10px; border-radius:8px; display:inline-block; }
...
def page_home():
    ...
    st.markdown("<p class='small'>Upload your resume (PDF / DOCX / TXT) or paste it — we'll extract and analyze it. Paste the Job Description (JD) you want to apply for. Get a percentage match, prioritized matched & missing keywords, and ATS formatting tips — instantly.</p>", unsafe_allow_html=True)
    st.markdown("<p style='margin-top:12px'><strong>Try it:</strong> Click <span style='background:#eef7ee; padding:4px 8px; border-radius:6px'>Scanner</span> in the top nav and upload a resume to begin.</p>", unsafe_allow_html=True)
...
def page_results():
    ...
    # Top metrics row (clickable)
    st.markdown('<div class="metric-row">', unsafe_allow_html=True)
    # Match Rate big card
    color = "#28a745" if r["score"] >= 70 else "#ffc107" if r["score"] >= 40 else "#dc3545"
    st.markdown(render_chip_html("Match Rate", f"{r['score']}%", color="#fff", link_id="section-overview"), unsafe_allow_html=True)
    # Searchability
    sc_color = "#f8d7da" if r["searchability_issues"] else "#e6f4ea"
    st.markdown(render_chip_html("Searchability", f"{r['searchability_issues']} issues to fix", color="#fff", link_id="section-search"), unsafe_allow_html=True)
    # Hard Skills
    hs_color = "#f8d7da" if r["hard_issues"] else "#e6f4ea"
    st.markdown(render_chip_html("Hard Skills", f"{r['hard_issues']} issues to fix", color="#fff", link_id="section-hard"), unsafe_allow_html=True)
    # Soft Skills
    ss_color = "#f8d7da" if r["soft_issues"] else "#e6f4ea"
    st.markdown(render_chip_html("Soft Skills", f"{r['soft_issues']} issues to fix", color="#fff", link_id="section-soft"), unsafe_allow_html=True)
    # Recruiter Tips
    rt_color = "#f8d7da" if r["recruiter_tips_count"] > 0 else "#e6f4ea"
    st.markdown(render_chip_html("Recruiter Tips", f"{r['recruiter_tips_count']} issues to fix", color="#fff", link_id="section-tips"), unsafe_allow_html=True)
    # Formatting
    fm_color = "#f8d7da" if r["formatting_issues"] else "#e6f4ea"
    st.markdown(render_chip_html("Formatting", f"{r['formatting_issues']} issues to fix", color="#fff", link_id="section-format"), unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)
    ...
    if ci["linkedin"]:
        st.markdown("<div class='small' style='margin-top:8px'>LinkedIn link found — good job!</div>", unsafe_allow_html=True)
    else:
        st.markdown("<div class='small' style='margin-top:8px'>Add LinkedIn profile for recruiter confidence.</div>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

    # Hard Skills section
    st.markdown('<div id="section-hard" class="section-box">', unsafe_allow_html=True)
    st.markdown("<div style='font-weight:700; margin-bottom:6px;'>Hard Skills</div>", unsafe_allow_html=True)
    hard = r["cat"]["hard"]
    if hard["matched"]:
        st.markdown("<div class='kv'>Top matched technical skills</div>", unsafe_allow_html=True)
        st.write(", ".join([f"{k} ({v})" for k, v in hard["matched"].items()]))
    else:
        st.markdown("<div class='small'>No technical skills detected from JD in resume.</div>", unsafe_allow_html=True)
    if hard["missing"]:
    ...
    # Soft Skills section
    st.markdown('<div id="section-soft" class="section-box">', unsafe_allow_html=True)
    st.markdown("<div style='font-weight:700; margin-bottom:6px;'>Soft Skills</div>", unsafe_allow_html=True)
    soft = r["cat"]["soft"]
    if soft["matched"]:
        st.markdown("<div class='kv'>Matched soft skills</div>", unsafe_allow_html=True)
        st.write(", ".join(list(soft["matched"].keys())))
    else:
        st.markdown("<div class='small'>No soft skills matched — consider adding collaboration, leadership, communication, etc.</div>", unsafe_allow_html=True)
    if soft["missing"]:
    ...
    # Recruiter Tips
    st.markdown('<div id="section-tips" class="section-box">', unsafe_allow_html=True)
    ...
    if tips:
        ...
        st.write("No specific tips found.")
    st.markdown("</div>", unsafe_allow_html=True)

    # Formatting
    st.markdown('<div id="section-format" class="section-box">', unsafe_allow_html=True)
    ...
    # Skills table (Resume vs JD counts) — show selected top items
    ...
    for term, jdcount in jd_top:
        ...
        safe_term = term
        st.markdown(f"<tr><td>{safe_term}</td><td>{rc}</td><td>{jdcount}</td></tr>", unsafe_allow_html=True)
    st.markdown("</table>", unsafe_allow_html=True)
    st.markdown("</div>", unsafe_allow_html=True)

    # Additional insights: Experience, Education, Measurable results, Tone, Word count
    st.markdown('<div class="section-box">', unsafe_allow_html=True)
    st.markdown("<div style='font-weight:700; margin-bottom:6px;'>Additional Insights</div>", unsafe_allow_html=True)
    secs = r["sections"]
    if secs["summary"]:
        st.markdown("• Summary section: <b>Found</b>", unsafe_allow_html=True)
    else:
        st.markdown("• Summary section: <b>Missing</b> — add a concise summary at the top.", unsafe_allow_html=True)
    if secs["experience"]:
        st.markdown("• Work Experience section: <b>Found</b>", unsafe_allow_html=True)
    else:
        st.markdown("• Work Experience section: <b>Empty</b> — add at least one listing (internship or project).", unsafe_allow_html=True)
    if secs["education"]:
        st.markdown("• Education section: <b>Found</b>", unsafe_allow_html=True)
    else:
        st.markdown("• Education section: <b>Missing</b>", unsafe_allow_html=True)
    ...
    # web presence
    wp = r["contact_info"]
    st.markdown(f"• LinkedIn: <b>{'Found' if wp['linkedin'] else 'Not found'}</b>", unsafe_allow_html=True)
    ...
    st.markdown("<br/>")
    if st.button("Scan another resume", key="scan_again"):
